...
def login():
    ...
    if form.validate_on_submit():
        ...
        login_user(user, remember=form.remember_me.data)  # login_user 函数提供的用户登录记住功能,可以将用户地cokkie保存到浏览器
        next_page = request.args.get('next')
        if not next_page or url_parse(next_page).netloc != '':  # netloc服务器位置或者是用户信息
...
def register():
    ...
    if form.validate_on_submit():  # post请求返回ture get请求返回flase post请求才会有用户输入的用户名和密码
...
def reset_password(token):
    if current_user.is_authenticated:
        return redirect(url_for('index'))
    user = User.verify_reset_password_token(token)  # 判断 连接中的key和数据库key是否相同 不同则跳转到首页
    if not user:
        return redirect(url_for('index'))
